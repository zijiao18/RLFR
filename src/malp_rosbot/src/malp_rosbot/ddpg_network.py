...
class ActorNetwork():
	def __init__(self,sess,name,
				time_step,obs_dim,
				vel_dim,dir_dim,act_dim,
				batch_size,lstm_state_dim,
				n_fc1_unit,n_fc2_unit,n_fc3_unit,
				learning_rate,tau,device,master_network=None):
		self.sess=sess
		self.name=name
		...
		with tf.device(device):
			# Actor network
			self.obs_in,self.dir_in,self.ind_in,self.act_out = self.build_network(rnn_scope=self.name)
			self.net_params=tf.trainable_variables()[self.net_param_offset:]	#all the weights and bias in actor network
			# Target network
			self.target_obs_in,self.target_dir_in,self.target_ind_in,self.target_act_out = self.build_network(rnn_scope=self.name+'_target')
			...
			if not master_network==None:
				...
				self.init_target_net_params = [self.target_net_params[i].assign(self.master_network.net_params[i]) 
												for i in range(len(self.master_network.net_params))]

				self.update_target_net_params = [self.target_net_params[i].assign(tf.multiply(self.master_network.net_params[i],self.tau)
	...
	def build_network(self,rnn_scope):
		...
		lvel_out=tf.layers.dense(inputs=fc3,
								units=1,
								activation=tf.nn.sigmoid,
								kernel_initializer=tf.keras.initializers.he_normal(),#tf.initializers.random_normal(0.0,0.001),
								bias_initializer=tf.initializers.zeros())

		avel_out=tf.layers.dense(inputs=fc3,
								units=1,
	...
	def update_master_network(self,obs_batch,dir_batch,ind_batch,act_grads):
	...
	def predict(self, obs_batch,dir_batch,ind_batch):
	...
	def predict_target(self,obs_batch,dir_batch,ind_batch):
	...
	def summary(self,obs_batch,dir_batch,ind_batch,act_grads):
...
class CriticNetwork():
	...
	def __init__(self,sess,name,
		...
		with tf.device(device):
			...
			self.ind_in,self.q_out)=self.build_network(rnn_scope=self.name)
			...
			self.target_ind_in,self.target_q_out)=self.build_network(rnn_scope=self.name+'_target')
	...
	def predict(self, obs_batch,dir_batch,ind_batch,act_batch):
	...
	def predict_target(self,obs_batch,dir_batch,ind_batch,act_batch):
	...
	def action_gradients(self,obs_batch,dir_batch,ind_batch,act_batch,ind_scalar):
		if ind_scalar==0:
	...
	def update_master_network(self,obs_batch,dir_batch,ind_batch,act_batch,tar_q_batch):
		self.sess.run(self.apply_grads, feed_dict={
				self.obs0_in: obs_batch[0],
				self.obs1_in: obs_batch[1],
				self.obs2_in: obs_batch[2],
	...
	def summary(self,obs_batch,dir_batch,ind_batch,act_batch,tar_q_batch):
