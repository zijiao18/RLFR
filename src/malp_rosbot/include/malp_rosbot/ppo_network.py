...
class ActorNetwork():
	def __init__(self,sess,name,time_step,obs_dim,vel_dim,dir_dim,act_dim,batch_size,
		...
		self.update_target_net_params = [self.target_net_params[i].assign(tf.multiply(self.net_params[i],
																					self.tau)
										+tf.multiply(self.target_net_params[i],
													1.0-self.tau))
										for i in range(len(self.target_net_params))]

		self.num_trainable_vars=len(self.net_params)+len(self.target_net_params)

		self.advantage_in=tf.placeholder(dtype=tf.float32,shape=())
		self.log_prob_old=tf.placeholder(dtype=tf.float32,shape=())
		self.surog_obj=tf.multiply(tf.div(self.act_out,self.log_prob_old),self.advantage_in)
		self.pg=tf.gradients(ys=self.surog_obj,xs=self.net_params)
...
class CriticNetwork():
	...
	def __init__(self,sess,name,time_step,obs_dim,vel_dim,dir_dim,act_dim,batch_size,
		...
		self.tb_loss=tf.summary.scalar(self.name+' loss',self.loss)
		...
		self.ind_scalar=0
		self.action_grads = tf.gradients(self.q_out, self.act0_in)
	...
	def action_gradients(self,obs_batch,vel_batch,dir_batch,ind_batch,act_batch):
		self.ind_scalar=ind_batch[0][0][0]
