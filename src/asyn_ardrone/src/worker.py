...
class Worker():
	def __init__(self,sess,name,init_pose,goal,master_actor,master_critic,training,tb_writer):
		self.sess=sess
		self.name=name
		self.state=State(goal=goal,pose=init_pose.copy(),obs_dim=360,obs_seq_len=4)
		self.pre_state=State(goal=goal,pose=init_pose.copy(),obs_dim=360,obs_seq_len=4)
	...
	def train(self):
		if not self.training:
			raise NameError("worker.train(): the worker is not in training mode...")
	...
	def interact_with_environment(self):
		rewards=0;
		ep_rwd_log=open('./save/log/'+self.name+'_ep_rwd_train.csv','wb')
		while not rospy.is_shutdown():
	...
	def evaluate(self):
		...
		if self.training:
			...
			print("worker.evaluate() is returned...")
			return
