...
class State():
	...
	def update_obsseq_event(self,lidar_ranges):
		#detect collision event based on normalized lazer ranges
	...
	def update_vel_dir_pose(self,velocity,pose,goal):
		self.pose[0,:]=pose
		self.vel[0,:]=velocity
		self.dir[0,:]=(goal[0,:]-pose[0,0:3])/np.linalg.norm(goal-pose[0,0:3])
...
class ReplayBuffer():
	...
	def add(self,sars):
		...
		if len(self.buf)<self.max_size:
			#print("sars","cur_state_obs: ", sars['cur_state'].obs[:,0:5],"next_state_obs: ",sars['next_state'].obs[:,0:5])
			#print("sars","cur_state_vel: ", sars['cur_state'].vel, "next_state_vel: ",sars['next_state'].vel)
			#print("sars","cur_state_pose: ", sars['cur_state'].pose, "next_state_pose: ",sars['next_state'].pose)
			#print("sars","cur_state_dir: ", sars['cur_state'].dir, "next_state_dir: ",sars['next_state'].dir)
