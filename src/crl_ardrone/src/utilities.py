...
class State():
	...
	def update_obs_dropout(self,feedback,goal,rate=0.5):
		self.event=feedback.event
		self.pose[0,:]=feedback.pose
		self.vel[0,:]=feedback.velocity
		self.dir[0,:]=(goal[0,:]-feedback.pose[0,0:3])/np.linalg.norm(goal-feedback.pose[0,0:3])

		#dropout lidar_range measures
		...
		#print("updated obs: ",self.obs)
	def update_obs_dropout_eval(self,feedback,goal,rate=0.5):
		self.event=feedback.event
		self.pose[0,:]=feedback.pose
		self.vel[0,:]=feedback.velocity
		self.dir[0,:]=(goal[0,:]-feedback.pose[0,0:3])/np.linalg.norm(goal-feedback.pose[0,0:3])

		#scale lidar_range measures by the factor of rate
		for i in xrange(len(feedback.lidar_ranges)):
