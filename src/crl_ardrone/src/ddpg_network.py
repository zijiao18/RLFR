...
class ActorNetwork():
	...
	def build_network(self,rnn_scope):
		...
		lstm = tf.nn.rnn_cell.DropoutWrapper(lstm, state_keep_prob=0.6)#was 0.6
		...
		fc1 = tf.layers.dropout(inputs=fc1,rate=0.2,training=self.training)#rate was 0.2
		...
		fc2=tf.layers.dense(inputs=fc1,
							...
							kernel_initializer=tf.keras.initializers.he_normal(),#tf.initializers.random_normal(0.0,0.001),
							bias_initializer=tf.initializers.zeros())
		fc2 = tf.layers.dropout(inputs=fc2,rate=0.2,training=self.training)#rate was 0.2
...
class CriticNetwork():
	...
	def build_network(self,rnn_scope):
		...
		lstm = tf.nn.rnn_cell.DropoutWrapper(lstm, state_keep_prob=0.6) #state_keep_prob was 0.6
		hs,c=tf.nn.static_rnn(cell=lstm,
							inputs=tf.unstack(lstm_in),
							dtype=tf.float32,
							scope=rnn_scope)
