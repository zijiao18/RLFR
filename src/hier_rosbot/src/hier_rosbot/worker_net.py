...
class WorkerActorNetwork():
	...
	def build_network(self, rnn_scope):
		...
		return obs_in, pose_in, goal_in, time_in, act_out
	...
	def predict_target(		
		...
		return self.sess.run(
			...
			feed_dict={
				...
				self.target_time_in: time_batch,
			}
		)
...
class WorkerCriticNetwork():
	...
	def build_network(self, rnn_scope):
		...
		return obs_in, pose_in, goal_in, time_in, act_in, q_out
